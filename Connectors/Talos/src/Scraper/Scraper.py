...
class Scraper:
    ...
    def getZeroDayData(self):
        try:
            scraper = cloudscraper.create_scraper()
            file_list = open("zeroDays.txt", "r")
            zeros_data = []
            for line in file_list:
                print("https://talosintelligence.com/vulnerability_reports/"+line.rstrip("\n"))
            ...
            with open('zerosData.json', 'w', encoding="utf-8")as f:
                json.dump(zeros_data, f, ensure_ascii=False, indent = 4)
    ...
    def getDisclosedsData(self):
        try:
            scraper = cloudscraper.create_scraper()
            discl_list = open("discloseds.txt", "r")
            discls_data = []

            for line in discl_list:
                line = line.rstrip("\n")
                strs = line.split(",")
                ...
                if("200" in str(req)):
                    ...
                    if(page):
                        ...
                        print(data)
                        discls_data.append(data)

            with open('disclosedsData.json', 'w', encoding="utf-8")as f:
                json.dump(discls_data, f, ensure_ascii=False, indent = 4)
    ...
    def scraping(self):
        ...
        self.getDiscloseds()
        self.getZeroDayData()
        self.getDisclosedsData()

#scraper = Scraper()
#scraper.getZeroDayList()
#scraper.getDiscloseds()
#scraper.getReputationIp()
#scraper.getZeroDayData()
#scraper.getDisclosedsData()
